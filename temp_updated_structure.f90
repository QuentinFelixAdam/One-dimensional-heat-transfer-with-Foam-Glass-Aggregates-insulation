...
REAL,DIMENSION(:),ALLOCATABLE :: list_FI
REAL :: d_10_FGA
...
ALLOCATE(list_temp_moy(24))
ALLOCATE(list_temp_amp(24))
ALLOCATE(list_FI(24))
...
rho_3 = 0.5*(rho_dry_3+(rho_dry_3*rho_dry_3+4000.0*theta_tot_3*rho_dry_3)**(0.5))
...
IF(((MOD(i-1,180*24)).EQ.0)) THEN
   ...
   WRITE(26,*) (i-1)*dt,intercept,temp_avg_FGA,Rayleigh_coeff,&
		 Air_avg,Air_amp,Tnew(Nz_2+1),Tnew(Nz_3-1),d_3,condu_3_eq,&
		 list_FI(m_loop6),air_heat_capa,air_thermal_exp,&
		 air_viscosity_k,condu_3_eq,perma_3,d_1,d_2,d_4
    CLOSE(26)
END IF
END IF
END IF
...
IF(1.EQ.0) THEN
   !IF(i-1.GE.498420) THEN ! 496800
      IF(((MOD(i-1,180*24)).EQ.0)) THEN
          WRITE(26,*) (i-1),temp_avg_FGA
      END IF
   !END IF
END IF
...
!UPDATE TEMPERATURE OF INTERFACE 2_3 MG20 to FGA
...
!CALCULATE FLUID PARAMETERS FOR CONVECTION IN FGA
   
   DO index_FGA = Nz_2+1,Nz_3-1
      temp_avg_FGA = temp_avg_FGA + Tnew(index_FGA)
   END DO
   
   temp_avg_FGA = REAL(temp_avg_FGA)/REAL(Nz_3 - 1.0 - Nz_2 - 1.0)
   
   air_heat_capa = (2.0*10**(-5.0)*temp_avg_FGA*temp_avg_FGA - &
   0.0049*temp_avg_FGA + 1.3012)*1000.0
   
   air_viscosity_k = (0.0001 * temp_avg_FGA*temp_avg_FGA + &
   0.0866*temp_avg_FGA + 13.285)*10**(-6.0)
   
   air_thermal_exp = (5.0*10**(-5.0)*temp_avg_FGA*temp_avg_FGA - &
   0.0143*temp_avg_FGA + 3.6947) * 10**(-3.0)
   
   condu_3_rad = 4.0*1.0*d_10_FGA*5.67*10**(-8.0)*(273.15 + temp_avg_FGA)**(3.0)
   
   condu_3_eq = condu_3 + condu_3_rad
   
   Rayleigh_coeff = 9.81 * air_thermal_exp * air_heat_capa * d_3 * &
   (Told(Nz_3-1) - Told(Nz_2+1)) * perma_3 /( air_viscosity_k * &
   condu_3_eq)
   
!LOOP TO UPDATE TEMPERATURE WITHIN LAYER 3 -- FGA

   DO j = Nz_2+1,Nz_3-1
	  
	  IF(Rayleigh_coeff.LT.39.4784176) THEN
	     k_eff(j) = condu_3_eq
      ELSE
	     k_eff(j) = condu_3_eq * (1.735 * LOG(Rayleigh_coeff) - 5.38)
	  END IF
     ...
     c_eff(j) = c_3_f*(1.0-exponentlatent) + &
     c_3_u*exponentlatent

      Tnew(j) = Told(j) + condu_3_eq*dt*rdz2*(Told(j+1) - 2.0*Told(j) + &
      Told(j-1))/(c_eff(j)*rho_3 + latentheat*d_T_th_uw(j))

   END DO
   
!UPDATE TEMPERATURE OF INTERFACE 3_4 -- FGA TO MG112
...
!UPDATE TEMPERATURE AT LAST NODE
   ...
   Tnew(Nz_5) = Tnew(Nz_5-1) + 0.0125*dz
